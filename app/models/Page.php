...
class Page
{
    ...
    public function get_dine_active_orders_pay($id) {
        $this->db->query('SELECT * FROM orders WHERE table_id = :id AND order_type = 1 AND DATE(created_at) =:date_val AND payment_status = 0');
         $this->db->bind(':id', $id);
         $this->db->bind(':date_val', date('Y-m-d'));
         $result = $this->db->resultSet();
         return $result;
    ...
    public function get_vendor_orders_settlements_dine($id) {
        $this->db->query('SELECT * FROM orders WHERE vendor_id = :id AND order_type = 1 AND settled = 0 AND DATE(created_at) !=:date_val AND transaction_id IS NOT NULL');
         $this->db->bind(':id', $id);
         $this->db->bind(':date_val', date('Y-m-d'));
         $result = $this->db->resultSet();
        return $result;
    ...
    public function get_vendor_orders_settlements_delivery($id) {
        $this->db->query('SELECT * FROM orders WHERE vendor_id = :id AND order_type = 0 AND settled = 0 AND DATE(created_at) !=:date_val AND transaction_id IS NOT NULL');
         $this->db->bind(':id', $id);
         $this->db->bind(':date_val', date('Y-m-d'));
         $result = $this->db->resultSet();
        return $result;
    ...
    public function get_vendor_orders_settlements_pickup($id) {
        $this->db->query('SELECT * FROM orders WHERE vendor_id = :id AND order_type = 2 AND settled = 0 AND DATE(created_at) !=:date_val AND transaction_id IS NOT NULL');
         $this->db->bind(':id', $id);
         $this->db->bind(':date_val', date('Y-m-d'));
         $result = $this->db->resultSet();
        return $result;
...
public function add_table($table_name)
{
    ...
    {   $time= date('Y-m-d H:i:s');
      ...
      public function dine_cart_item_del($id){
        // Prepare Query
        $this->db->query('DELETE FROM cart_dine WHERE user_id =:uid AND item_id = :id');
    ...
    public function update_password($phone, $pass)
    {
        ...
        $this->db->query('UPDATE auth SET password = :pass WHERE phone = :phone AND type=:utype');
    ...
    {
        $this->db->query('SELECT * FROM orders where user_id =:uid AND coupon_id = :id');
        $this->db->bind(':id', $id);
        $this->db->bind(':uid', $_SESSION['rexkod_user_id']);
        return $this->db->single();
    }
